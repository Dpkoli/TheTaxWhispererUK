...
import { runBusinessRatesComputation } from "@/app/compute/business-rates/actions";

type ComputationResult = Awaited<ReturnType<typeof runBusinessRatesComputation>>;
...
function downloadCsv(result: ComputationResult) {
  ...
  a.href = url;
  a.download = `business-rates-computation-${result.rateTableVersion.taxYear}.csv`;
  a.click();
      ...
      <Card>
        <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-4">
          <div className="flex-1 min-w-[220px]">
            <label htmlFor="rateable-value" className="text-sm font-medium text-ink/80">
              Rateable value
            </label>
            <input
              id="rateable-value"
              ...
              step="0.01"
              value={rateableValue}
              onChange={(event) => setRateableValue(event.target.value)}
          ...
          <label className="flex items-center gap-2 text-sm text-ink/80">
            <input
              type="checkbox"
              checked={isRhl}
              onChange={(event) => setIsRhl(event.target.checked)}
